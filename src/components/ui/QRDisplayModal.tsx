...
export default function QRDisplayModal({ record, upiString, onClose }: Props) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `qr-${record.name}-₹${record.amount}.svg`;
    a.click();
  ...
  const handleShare = async () => {
    if (navigator.share) {
      await navigator.share({
        title: `Pay ₹${record.amount} to ${record.name}`,
        text: `UPI: ${record.upiId}`,
        url: upiString,
      });
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="relative w-full max-w-sm bg-white rounded-t-3xl sm:rounded-2xl px-6 pt-5 pb-8 shadow-2xl z-10 animate-slide-up text-center">
        ...
        <p className="text-slate-400 text-xs mb-1 font-mono">{record.upiId}</p>
        ...
        <div className="my-4">
          ...
          {record.note && (
            <p className="text-slate-400 text-xs mt-1">{record.note}</p>
          )}
        </div>
