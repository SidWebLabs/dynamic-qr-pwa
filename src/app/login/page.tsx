...
import { FiUser, FiLock, FiEye, FiEyeOff, FiArrowRight, FiPhone, FiAlertCircle } from "react-icons/fi";
...
function LoginForm() {
    ...
    const [fieldErrors, setFieldErrors] = useState<{ id?: string; pin?: string }>({});
    ...
    const validateIdentifier = (val: string) => {
        if (!val.trim()) return "Username or mobile number is required.";
        const isMobile = /^\d+$/.test(val.trim());
        if (isMobile && val.trim().length !== 10) return "Mobile number must be 10 digits.";
        if (!isMobile && val.trim().length < 3) return "Username must be at least 3 characters.";
        if (!isMobile && /[^a-zA-Z0-9._]/.test(val.trim()))
            return "Only letters, numbers, dots or underscores allowed.";
        return "";
    };
    ...
    const handleSubmit = () => {
        ...
        if (idErr || pinErr) {
            setFieldErrors({ id: idErr || undefined, pin: pinErr || undefined });
            triggerShake();
            return;
        }
        setSubmitting(true);
        ...
        setFieldErrors({});
        setTimeout(() => {
            const result = login(identifier, pin.join(""));
            if (result.success) {
    ...
    const canSubmit = identifier.trim().length >= 3 && pinFilled && !submitting;
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 py-10">
                ...
                {/* Card */}
                <div className={`w-full max-w-sm bg-white/10 backdrop-blur-xl rounded-3xl p-6 sm:p-8 border border-white/15 shadow-2xl ${shake ? "animate-shake" : ""}`}>
                    {/* Global error */}
                    {error && (
                    ...
                    <div className="mb-5">
                        ...
                        <div className="relative">
                            <FiUser size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/40" />
                            <input
                                type="text"
                                ...
                                onChange={(e) => {
                                    ...
                                    setError(null);
                                    setFieldErrors((p) => ({ ...p, id: validateIdentifier(e.target.value) || undefined }));
                                }}
                                onKeyDown={(e) => { if (e.key === "Enter") pinRefs.current[0]?.focus(); }}
                                className={`w-full pl-10 pr-4 py-3.5 rounded-xl bg-white/10 border text-white placeholder-white/30 text-sm focus:bg-white/15 transition-colors outline-none ${fieldErrors.id ? "border-red-400" : "border-white/20 focus:border-blue-400"
                                    }`}
                            />
                        </div>
                        {fieldErrors.id && (
                            <p className="text-red-400 text-xs mt-1.5 flex items-center gap-1">
                                <FiAlertCircle size={11} /> {fieldErrors.id}
                            </p>
                        )}
                    </div>
